...
export default function AnalysisPage() {
    ...
    const totalMarketCap = useMemo(() => sectors.reduce((a, s) => a + s.weight, 0) || 1, [sectors]);
    ...
    const displaySectors = useMemo(() => sectors.map(sec => {
        ...
        const sectorTotalCap = stocks.reduce((a, s) => a + s.cap, 0) || 1;
        const dynamicChangePercent = stocks.reduce((a, s) => a + s.changePercent * (s.cap / sectorTotalCap), 0);
        
        // 가시적인 '시장 점유율' 표시를 위해 전체 대비 백분율(0~100)로 변환합니다.
        const normalizedWeight = (sec.weight / totalMarketCap) * 100;
        
        return { ...sec, stocks, changePercent: dynamicChangePercent, weight: normalizedWeight };
    }), [sectors, isSessionActive, marketStatus, totalMarketCap]);
    ...
    useEffect(() => { fetchSectors(market); const i = setInterval(() => fetchSectors(market), 60000); return () => clearInterval(i); }, [market, fetchSectors]);
